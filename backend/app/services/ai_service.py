...
🚨🚨🚨 ПРАВИЛО #2 - ФОРМАТИРОВАНИЕ (ОБЯЗАТЕЛЬНОЕ!) 🚨🚨🚨

▶▶▶ КАЖДОЕ предложение НАЧИНАЕТСЯ С НОВОЙ СТРОКИ (\n)
▶▶▶ МЕЖДУ БЛОКАМИ информации - ПУСТАЯ СТРОКА (\n\n)

ПРАВИЛА ФОРМАТИРОВАНИЯ:
1. Короткий ответ (1-3 предложения):
   - Каждое предложение с новой строки (\n)
   - БЕЗ пустых строк между предложениями

2. Длинный ответ (списки, перечисления):
   - Вводная фраза
   - ПУСТАЯ СТРОКА (\n\n)
   - Список с "-" или "1."
   - ПУСТАЯ СТРОКА (\n\n)
...
3. Оформление:
   - **Жирный текст** для важной информации (цены, названия тарифов)
   - Списки: "- " или "1. " для перечислений
   - БЕЗ URL (www., http://, https://)

❌ НЕПРАВИЛЬНО (сплошной текст):
"Наша компания предлагает разнообразные курсы по изучению китайского языка с акцентом на разговорную практику. Мы стремимся создать идеальную программу. Есть ли что-то конкретное, что вас интересует?"

✅ ПРАВИЛЬНО (каждое предложение с новой строки):
"Наша компания предлагает разнообразные курсы по изучению китайского языка.
Мы делаем акцент на разговорную практику.
Создаём идеальную программу под ваши цели.

Есть ли что-то конкретное, что вас интересует?"

❌ НЕПРАВИЛЬНО (список без пустых строк):
"У нас работают опытные преподаватели.
- Высокая квалификация
- Индивидуальный подход
- Эффективные методики
Хотите узнать больше?"

✅ ПРАВИЛЬНО (список с пустыми строками):
"У нас работают опытные преподаватели.

- Высокая квалификация и большой опыт
- Индивидуальный подход к каждому ученику
- Эффективные методики обучения

Хотите узнать о квалификации конкретных учителей?"
...
"расскажи о компании" → {"response": "Наша компания предлагает курсы по изучению китайского языка.\nМы делаем акцент на разговорную практику.\nСоздаём программы под ваши цели.\n\nЧто именно вас интересует?", "name": "0", "phone": "0"}
"учителя" → {"response": "У нас работают опытные преподаватели.\n\n- Высокая квалификация\n- Индивидуальный подход\n- Эффективные методики\n\nХотите узнать о квалификации конкретных учителей?", "name": "0", "phone": "0"}
...
Никаких "дайте знать", "сообщите", "обращайтесь" - ТОЛЬКО ВОПРОС! """
        ...
        trimmed_history = self._trim_history_to_fit_tokens(
            ...
            max_context_tokens=12000,  # 12k токенов для контекста
            max_response_tokens=3000  # 3k токенов для ответа (достаточно для развернутых консультаций)
        )
        ...
        max_output_tokens = 3000  # Наш лимит (без ограничений для качественных ответов)
        ...
        our_output_limit = 3000       # Наш лимит ответа
                    ...
                    async with httpx.AsyncClient(
                        ...
                        response = await client.post(
                            ...
                            json={
                                ...
                                "messages": messages,
                                "temperature": 0.7,
                                "max_tokens": 3000,  # Лимит токенов для ответа (позволяем развернутые консультации)
                        ...
                        # Если ответ обрезан - это означает РЕАЛЬНОЕ переполнение контекста
                        # В этом случае обрезаем историю, оставляя только последние 2 сообщения
                        if finish_reason == "length" and parse_attempt < 2:
                            logger.warning(f"⚠️ Ответ обрезан из-за переполнения контекста (finish_reason=length), попытка {parse_attempt + 1}/3")
                            logger.warning(f"   Контекст использовал слишком много токенов. Обрезаю историю до последних 2 сообщений.")

                            # Оставляем только последние 2 сообщения из истории для сохранения контекста
                            if len(conversation_history) > 2:
                                trimmed_history_aggressive = conversation_history[-2:]
                            else:
                                trimmed_history_aggressive = conversation_history
