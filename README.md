...
- `OPENAI_PROXY_URL`, `OPENAI_PROXY_USERNAME`, `OPENAI_PROXY_PASSWORD` - данные прокси (опционально)
  - Поддерживается HTTP/HTTPS прокси
  - Поддерживается Shadowsocks прокси (ss://) - автоматически создается локальный SOCKS5 прокси
...
- ✅ Поддержка Shadowsocks прокси через ss-local
- ✅ Markdown форматирование ответов бота
- ✅ Настройка цветовой схемы виджета через админ панель
- ✅ Автоматическое расширение виджета с сообщением через 5 секунд
...
Коммиты в этом проекте подписываются настройками Git. Укажите имя и почту, которые должны фигурировать в истории изменений:
...
Если нужно изменить автора для одного коммита, можно использовать опцию `--author`:

```bash
git commit --author="Имя <email@example.com>" -m "сообщение"
```

Актуального автора последнего коммита можно увидеть командой `git log -1 --pretty=format:'%an <%ae>'`.
